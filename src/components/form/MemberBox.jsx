...
const MemberBox = () => {
    ...
    // URL에서 추출한 urlKey를 이용하여 팀매칭 아이디 가져오기
    useEffect(() => {
        fetch(`${window.location.origin}/${urlKey}`)
            .then((response) => response.json())
            .then((data) => {
                if (data.success) {
                    setMatchingId(data.matchingId);  // 팀매칭 아이디 설정
                } else {
                    console.error("팀매칭 아이디 불러오기 실패:", data.message);
                    alert("팀매칭 정보를 불러올 수 없습니다.");
                }
            })
            .catch((error) => {
    ...
    // 채팅 시작 버튼 클릭 핸들러
    const handleStart = () => {
        console.log("이름:", name, "소속:", organization, "팀매칭:", matchingId);

        // 백엔드 API로 팀매칭 ID와 사용자 정보를 전송
